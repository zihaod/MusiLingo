...
class MSDDataset(BaseDataset):
    def __init__(self, processor, mp3_file_folder, split, 
                 msd="/jmain02/home/J2AD007/txk47/yxm46-txk47/yinghao/MUVI/model/msd"):
        assert(split in ["train", "valid", "test"])
        self.raw_file_folder = mp3_file_folder
        try:
            self.msd_dataset = load_from_disk(msd)[split]
        except:
            login(token="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")
            self.msd_dataset = load_dataset("seungheondoh/LP-MusicCaps-MSD")[split]
            self.msd_dataset.save_to_disk(msd)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            res_dict["audio"] = audio.squeeze(0)
